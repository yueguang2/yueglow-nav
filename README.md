...
- 主题管理：内置微信绿和海洋蓝两个只读系统预设，自定义主题支持深色/浅色配色、实时预览、动态背景和背景模糊开关。
...
- 系统预设：微信绿和海洋蓝由应用自动维护，只能激活，不能编辑或删除。
...
测速会并发检测当前站点启用的全部链接，并选择响应最快的可用链接。探测会跳过 `localhost`、回环地址和链路本地地址；普通内网 IP 地址（例如 `192.168.x.x`、`10.x.x.x`、`172.16-31.x.x`）会参与测速，适合 NAS、软路由、家庭服务等场景。

如果只有一条启用链接，系统会直接使用该链接，不进行测速。如果多条链接都没有通过测速，会退回到排序第一的启用链接。
...
- 后台主题：访问 `/admin/themes` 可以管理主题配色。默认内置“微信绿”和“海洋蓝”两个系统预设，系统预设不可编辑或删除。
- 自定义主题：新增主题时可选择“微信简洁”或“海洋经典”界面风格，颜色、动态背景和背景模糊仍可独立配置。
...
代码类型中预留了 `glass` 和 `minimal` 风格，但当前界面表单和数据库读取逻辑实际只使用 `wechat` 与 `classic` 两种风格。
...
| 名称 | 标识符 | 界面风格 | 说明 |
| --- | --- | --- | --- |
| 微信绿 | `wechat` | 微信简洁 | 默认系统预设，浅灰底、绿色强调，更偏列表化和紧凑管理体验。 |
| 海洋蓝 | `ocean` | 海洋经典 | 系统预设，保留圆润卡片、阴影和动态背景，更接近旧版视觉。 |
...
主题表会自动迁移新增 `ui_style` 字段，旧数据默认使用 `wechat` 风格。分类、站点表也会自动迁移置顶字段。启动时还会自动维护微信绿、海洋蓝两个系统预设，并确保始终只有一个激活主题。
